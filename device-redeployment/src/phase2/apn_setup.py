...
# RULED OUT, 2026-09-18 (SOG07): a hypothesis briefly lived here — that
# the entry going completely missing after configure_apn() reported
# success (client screenshot: the APN list showed only the two
# pre-existing carrier entries, no "rakuten.jp" at all — not even a
# version with wrong MCC/MNC) was caused by the second `input text` call
# in use_text_entry_for_numeric's probe-then-rest sequence still being an
# uncommitted IME composing span when the dialog's confirm button was
# tapped. Added a 1-second delay there and had the client retest on real
# hardware: same result, entry still completely absent. Removed rather
# than kept "just in case" — this was NOT the cause. See
